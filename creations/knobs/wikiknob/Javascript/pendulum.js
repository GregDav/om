...
$('#innerknob').css({
  transform: 'rotate(' + innerDeg + 'deg)' 	
	...
	var transition = langDeg-parseInt(langDeg);
	if(transition>.5){
		transition=.5-(transition-.5);
	}
	transition=transition*2;
		//$('#experienceContainer').css({'backgroundImage':'url(thumb/'+parseInt(langDeg)+'.jpg)'});
	if(buffer==false){
		grabber(targetlist[parseInt(langDeg)]);

		buffer=true;
	} 
	
	//$('#experienceContainer').css({'opacity':transition.toFixed(2)});
	innerknob();
