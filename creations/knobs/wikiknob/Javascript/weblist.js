...
fetch(linksurl)
    ...
    .then(function(response) {
		...
		targetlist = targetlist.concat(selectlinks);
			$('#handlein').css({'width': 1+targetlist.length/100+'vh', 'height' :1+targetlist.length/100+'vh'}); 
	console.log(1+target.length/100+'vh');
		$("#encoder").html('<p>'+String(targetlist)+'</p>');
    })
    .catch(function(error){console.log(error);});
}
